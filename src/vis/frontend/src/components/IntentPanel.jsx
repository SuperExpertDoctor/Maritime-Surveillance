...
export default function IntentPanel({ frame, readOnly = false, connectionStatus = "connected", selection, onClearSelection }) {
  ...
  const canWrite = !readOnly && connectionStatus === "connected" && Boolean(frame?.episode_id)
    && frame.runtime_status !== "finished";
  const intentBusy = submitting || ["queued", "unknown"].includes(command?.status);
  const runtimeBusy = runtimeSubmitting || ["queued", "unknown"].includes(runtimeCommand?.status);
  ...
  useEffect(() => {
    if (submitting || !command?.command_id || !["queued", "unknown"].includes(command.status)) return undefined;
    let stopped = false;
    let timer;
    const poll = async () => {
      try {
  ...
  useEffect(() => {
    if (runtimeSubmitting || !runtimeCommand?.command_id || !["queued", "unknown"].includes(runtimeCommand.status)) return undefined;
    let stopped = false;
    let timer;
    const poll = async () => {
      try {
  ...
  }, [runtimeCommand, runtimeSubmitting]);
  ...
  const submit = async (event) => {
    ...
    if (!canWrite || intentBusy || !draft.bbox) return;
    setSubmitting(true);
    setError("");
    ...
    try {
      ...
      if (!response.ok) throw Object.assign(new Error(result.message || result.error_code || "重点区命令未入队"), { rejected: true });
      setCommand({ command_id: result.command_id, status: result.status });
      if (editingId) setEditingId(null);
    } catch (submitError) {
      setCommand({ command_id: id, status: submitError.rejected ? "rejected" : "unknown" });
  ...
  const cancelIntent = async (intent) => {
    ...
    try {
      ...
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw Object.assign(new Error(result.message || result.error_code || "取消命令未入队"), { rejected: true });
      setCommand({ command_id: result.command_id, status: result.status });
    } catch (cancelError) {
      setCommand({ command_id: id, status: cancelError.rejected ? "rejected" : "unknown" });
  ...
  const sendRuntimeCommand = async (operation) => {
    if (!canWrite || runtimeBusy) return;
    ...
    try {
      ...
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw Object.assign(new Error(result.message || result.error_code || "运行命令未入队"), { rejected: true });
      setRuntimeCommand(result);
    } catch (runtimeError) {
      setRuntimeCommand({ command_id: id, status: runtimeError.rejected ? "rejected" : "unknown" });
      ...
      {!readOnly && (
        <form className="intent-form" onSubmit={submit}>
          ...
          <div className="intent-form-actions">
            <button type="submit" className="primary-action" disabled={!canWrite || !draft.bbox || intentBusy}><Send size={13} />{editingId ? "提交修改" : "提交重点区"}</button>
