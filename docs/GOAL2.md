...
4. **AIS 信号军民判别**：UAV 跟踪目标后捕获 AIS 信号，通过对比 AIS 位置与光电雷达推算的真实位置区分II 类船舶与I 类船舶
...
class Ship:
    ...
    vessel_class: Literal["type_i", "type_ii"]  # 由 AIS 判别后确定
...
```
 ...
 驱逐舰: 中型船舶图标，海军灰
 I 类船舶（AIS 判别后）: 小型船舶图标，白色/浅蓝，不显示跟踪框
...
> 这是 GOAL2 的核心新功能之一。当 UAV 跟踪到目标后，系统自动捕获目标的 AIS 信号，通过对比 AIS 自报位置与 UAV 推算的真实位置，区分II 类船舶与I 类船舶。I 类船舶不予跟踪，释放 UAV 资源。

### 5.1 AIS 信号模型

```
AIS (Automatic Identification System):
  I 类船舶强制广播: 位置、航速、航向、船名、MMSI、船型
  II 类船舶: 可选择性关闭 AIS 或广播虚假位置（战术欺骗）
  I 类船舶: 持续广播真实 AIS 信号
...
AIS 信号特性:
  ...
  - I 类船舶 AIS 位置误差: ± 1 cell（GPS 精度 + AIS 报告延迟）
  - II 类船舶 AIS 特性:
    方案 A（关闭 AIS）: 无 AIS 信号 → 直接判定为II 类船舶
    方案 B（广播虚假位置）: AIS 位置与真实位置偏差 > 2 cells → 判定为II 类船舶
    方案 C（正常广播）: II 类船舶伪装I 类船舶，AIS 位置 = 真实位置 → 需额外判别
```
...
Step 4: I 类船舶处理:
   if 判定为I 类船舶:
     ...
     e. 日志: "目标 {id} 判定为I 类船舶 (AIS: {mmsi}, {ship_name})，放弃跟踪"
     f. 可视化: 目标图标变为白色，持续 30 秒后从跟踪显示中移除

Step 5: II 类船舶处理:
   if 判定为II 类船舶:
     a. 继续跟踪
     b. 标记 ship.vessel_class = "type_ii"
     c. 日志: "目标 {id} 判定为II 类船舶 (AIS偏差={distance} cells)，持续跟踪"
...
```
 ...
 Canvas 上:
   - 判定为II 类船舶: 目标上方显示 ⚓ 红色锚图标
   - 判定为I 类船舶: 目标上方显示 🚢 灰色商船图标，渐隐后移除
...
- [ ] `src/utils/ais_discriminator.py`：`class AISDiscriminator`
  ...
  - `class DiscriminateResult`: vessel_class, confidence, reason
- [ ] 修改 `src/env/uav_entity.py`：集成 AIS 判别触发 + 结果处理
- [ ] 修改 `src/schedule/state_manager.py`：I 类船舶判定后释放跟踪区 + 释放 UAV
...
- [ ] AIS 判别结果渲染：II 类船舶锚图标 / I 类船舶商船图标
- [ ] 雷云规避状态指示器（Level 1/2/3）
- [ ] 目标驶离动画（渐隐 + 标注）

### Phase 6：端到端验证
- [ ] 多基地随机初始化 + 容量约束场景测试
- [ ] AIS 判别准确率测试（II 类船舶/I 类船舶各 20 个以上测试用例）
...
| # | 测试场景 | 预期效果 | 验证方式 |
|---|---------|---------|---------|
| A1 | I 类船舶 AIS 匹配 | AIS 位置 = 推算位置 → 判定为I 类船舶 → 放弃跟踪 | 单元测试 |
| A2 | II 类船舶 AIS 不匹配 | AIS 位置偏差 > 2 cells → 判定为II 类船舶 → 继续跟踪 | 单元测试 |
| A3 | II 类船舶关闭 AIS | 无 AIS 信号 → 判定为II 类船舶 → 继续跟踪 | 单元测试 |
| A4 | 判别延迟 | 首次跟踪后 2 分钟执行判别 | 时间戳验证 |
| A5 | I 类船舶释放 UAV | 判定为I 类船舶后 UAV 恢复搜索（非 idle） | 状态转换验证 |
...
| E4 | I 类船舶正确释放 | I 类船舶判定后 UAV 100% 释放 | 状态转换日志 |
