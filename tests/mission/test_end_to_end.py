...
def test_runtime_entities_expose_only_final_class_and_ais_contract(scenario_factory):
    engine = scenario_factory.engine("mixed-ais", seed=42)
    ship = engine.ships[0]

    assert hasattr(ship, "vessel_class")
    assert hasattr(ship, "ais_enabled")
    legacy_attributes = ("truth_" + "identity", "ais_" + "mode")
    assert all(not hasattr(ship, name) for name in legacy_attributes)
...
def test_fixture_factory_covers_all_eight_scenarios(scenario_factory):
    ...
    assert all(
        len(engine.ships) == engine.config.ship.population.total_count
        for engine in engines.values()
    )
    assert all(
        ship.vessel_class == "type_i"
        for ship in engines["all-type-i"].ships
    )
    assert not any(
        ship.ais_signal is not None
        for ship in engines["silent-type-ii"].ships
...
def test_ais_contacts_start_unknown_and_disguised_type_ii_needs_observation(scenario_factory):
    engine = scenario_factory.engine("disguised-type-ii", seed=42)
    target_count = sum(ship.vessel_class == "type_ii" for ship in engine.ships)
    contacts = engine.allocator.sm.contacts.list_snapshots()

    assert target_count > 0
    assert contacts
    assert all(contact.vessel_class == "unknown" for contact in contacts)
